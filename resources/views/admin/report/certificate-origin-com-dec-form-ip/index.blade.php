    ...
    <div class="col-12">
        ...
        <div class="card">
            <div class="card-body justify-content-start">
                <form id="" action="{{route('admin.report_List_certificate_origin_com_dec_form_ip_invoice')}}" class="row d-flex justify-content-between align-items-end">
                    ...
                    <div class="col-xl-1 col-sm-6 col-6 mt-4 mt-md-0">
                        <div class="d-flex justify-content-start align-items-start">
                            <a href="{{route('admin.report_List_certificate_origin_com_dec_form_ip_invoice')}}" class="btn btn report-tab-unactive"
                ...
                <div class="col-xl-2 col-sm-6 col-6 mt-6 pt-2 ">
                        <div class="d-flex justify-content-start align-items-start">
                            <a href="{{ URL::to('/panel/report/certificate-origin-com-dec-from-ip/add') }}" class="" target="_blank">
                            <button type="submit" class="btn btn report-tab-active"
            ...
            <div class="card-body p-0">
                <div class="table-responsive active-projects style-1">
                    ...
                    <table id="empoloyees-tblwrapper" class="table ">
                        ...
                        <tbody>
                            @foreach ($CertificateOriginComDecFormIp as $key => $value )
                            <tr>
                                <td><span>{{$value->id ?? ""}}</span></td>


                                <td>
                                    <span>{{$value->certificate_origin_com_decs_from_ip_invoices ?? ""}}</span>
                                </td>
                                <?php
                                 if (isset($certificate_origin_com_decs_from_ip_invoices->team_user_id)) {
                                    # code...
                                    $memberName = App\Models\team\TeamUser::where('id',$certificate_origin_com_decs_from_ip_invoices->team_user_id)->first('user_name');
                                }
                                ?>
                                ...
                                <button class="btn btn-sm report-tab-active thirdpartyIdForFormResubmit" style="font-size: 10px;" href="javascript:void(0)"  data-thirdparty="{{ $value->id }}">
                                    ...
                                    <a  class="btn btn-sm report-tab-active" style="font-size: 10px;" href="{{ URL::to('/panel/report/certificate-origin-com-dec-from-ip/view/'.$value->id) }}"  target="_blank" title="View Reports">
                                        View
                                    </a>
                                    <a  class="btn btn-sm report-tab-active" style="font-size: 10px;"  href="{{ URL::to('/panel/report/certificate-origin-com-dec-from-ip/edit/'.$value->id) }}"  target="_blank" title="Edit Reports">
                                        Edit
                                    </a>
                                ...
                                <a  class="btn btn-sm report-tab-active" style="font-size: 10px;" href="{{ URL::to('/panel/report/certificate-origin-com-dec-from-ip/activity/'.$value->id) }}" class="" target="_blank"  title="View Activity">
                               Activity
                                </a>
           ...
           confirmButtonText: 'Yes, re-submit!'
               ...
               if (result.isConfirmed) {
                   ...
                   $.ajax({
                       ...
                       url: "{{ route('admin.form_certificate_origin_com_dec_form_ip_invoice_resubmit') }}",
        ...
        $('.thirdpartyIdForForComplete').on('click', function() {
               ...
               if (result.isConfirmed) {
                   ...
                   $.ajax({
                       ...
                       url: "{{ route('admin.form_certificate_origin_com_dec_form_ip_invoice_completed') }}",
