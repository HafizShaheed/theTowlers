                                ...
                                <td class="text-center space-between ">
                                    ...
                                    switch ($result['table_name']) {
                                        case 'canada customer invoice':
                                           $urlParams = route('team.view_custom_canda_invoice', ['id' => $result['id']]);
                                            break;
                                        case 'certificate origins':
                                          
                                        $urlParams = route('team.view_certificate_origins_invoice', ['id' => $result['id']]);
                                            break;
                                        case 'Certificate Origin Com Decs':
                                            $urlParams = route('team.view_certificate_origin_com_dec_invoice', ['id' => $result['id']]);
                                            break;
                                        case 'Certificate Origin Com Decs from A':
                                        $urlParams = route('team.view_certificate_origin_com_dec_form_a_invoice', ['id' => $result['id']]);
                                            break;
                                        case 'Certificate Origin Com Decs from IPs':
                                            $urlParams = route('team.view_certificate_origin_com_dec_form_ip_invoice', ['id' => $result['id']]);
                                            break;
                                        case 'Certificate Origin No 627120s':
                                        $urlParams = route('team.view_certificate_origin_no627120_invoice', ['id' => $result['id']]);
                                            break;
                                        case 'Exporter Textile Declearations':
                                        $urlParams = route('team.view_exporter_textile_declearation_invoice', ['id' => $result['id']]);
                                            break;
                                        case 'Form 59 A invoice':
                                            $urlParams = route('team.view_form_59_a_invoice', ['id' => $result['id']]);
                                            break;
                                       default;
                                    }
                                @endphp
